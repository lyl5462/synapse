...
to find under the installed ``synapse`` package directory are actually
...
1. Walk up from the running ``synapse`` package directory looking for a
   sibling ``identity/`` (the repo-checkout / ``pip install -e .`` path).
2. Fall back to ``<synapse-package-root>/identity/<file>`` (the
...
class TestIdentityTemplatesAreShipped:
    ...
    @pytest.mark.parametrize("source_path", REQUIRED_IDENTITY_SOURCES)
    def test_identity_template_target_under_synapse_identity(
        self, force_include: dict[str, str], source_path: str
    ) -> None:
        """Wheel-side target must live under ``synapse/identity/`` so the
        ...
        assert target.startswith("synapse/identity/"), (
            f"{source_path!r} → {target!r}: target must be under "
            f"'synapse/identity/' so it ends up next to the "
            f"installed synapse package directory."
        )
        relative = source_path[len("identity/") :]
        assert target == f"synapse/identity/{relative}", (
            f"{source_path!r} → {target!r}: target should mirror the "
            f"source layout exactly (expected 'synapse/identity/{relative}')."
        )
...
class TestWizardCanResolveBundledTemplate:
    ...
    def test_resolves_soul_md_example_in_repo(self) -> None:
        from synapse.setup.wizard import _resolve_identity_template
