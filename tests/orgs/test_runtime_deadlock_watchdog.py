...
``synapse.orgs.command_tracker`` to make sure the re-export from
``synapse.orgs.runtime`` still works for external callers.
"""
...
from synapse.orgs.command_tracker import (
    UserCommandTracker as TrackerFromModule,
)
from synapse.orgs.models import NodeStatus
from synapse.orgs.runtime import OrgRuntime, UserCommandTracker


class TestUserCommandTrackerModuleSplit:
    """The class lives in :mod:`synapse.orgs.command_tracker` now but the
