...
2. **BUG-C1**: ``audit_logger._global_audit`` is a process-wide
   ...
   Hot-reload or ``SYNAPSE_AUDIT_LOG_PATH`` could change
...
from synapse.core.policy_v2 import audit_chain, global_engine, hot_reload
from synapse.core.policy_v2.hot_reload import PolicyHotReloader
...
class TestBugC1AuditLoggerSingletonRefresh:
    ...
    def test_env_override_audit_path_takes_effect_on_rebuild(
        ...
        """End-to-end: ``SYNAPSE_AUDIT_LOG_PATH`` set on a rebuild
        ...
        monkeypatch.setenv("SYNAPSE_AUDIT_LOG_PATH", str(second_log))
        global_engine.rebuild_engine_v2(yaml_path=policies)
        al.get_audit_logger().log(
