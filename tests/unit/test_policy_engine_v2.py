...
class TestSafetyImmunePathBoundary:
    ...
    **C8 注**：早期版本用 ``/etc/ssh`` 与 ``C:\\ProgramData\\Synapse`` 做
    ...
    def test_windows_backslash_normalized(self, tmp_path: Path) -> None:
        engine = PolicyEngineV2()
        ctx = _ctx(tmp_path, safety_immune_paths=("D:\\TestLab\\Synapse",))
        d = engine.evaluate_tool_call(
            ToolCallEvent(
                tool="write_file",
                params={"path": "D:\\TestLab\\Synapse\\config.yaml"},
            ),
            ctx,
        )
        assert d.safety_immune_match is not None
    ...
    def test_unc_path_normalized(self, tmp_path: Path) -> None:
        ...
        from synapse.core.policy_v2.engine import _path_under
    ...
    def test_double_star_anchor_matches_descendants(self) -> None:
        ...
        """
        from synapse.core.policy_v2.engine import _path_under, _strip_glob_anchor
...
class TestExtractRiskSignal:
    ...
    def test_real_risk_intent_result_extracted(self) -> None:
        from synapse.core.policy_v2.engine import _extract_risk_signal
    ...
    def test_dict_with_strenum_value(self) -> None:
        ...
        from synapse.core.policy_v2.engine import _extract_risk_signal
        from synapse.core.risk_intent import RiskLevel
...
def _src_explicit():
    ...
    from synapse.core.policy_v2 import DecisionSource
