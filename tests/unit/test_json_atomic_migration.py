...
def test_orchestrator_persist_uses_safe_json_write(tmp_path, monkeypatch):
    ...
    from synapse.utils import atomic_io as _io
    ...
    monkeypatch.setattr("synapse.agents.orchestrator.safe_json_write", spy, raising=False)
    ...
    from synapse.agents import orchestrator as _o
...
@pytest.mark.parametrize(
    ...
    [
        ("synapse.core.proactive", ["_load", "_save"]),
        ("synapse.core.user_profile", ["_load_state", "_save_state"]),
        ("synapse.sessions.user", ["_load_users", "_save_users"]),
        ("synapse.channels.adapters.telegram", ["_load_paired_users", "_save_paired_users"]),
        ("synapse.channels.media.storage", ["_load_index", "_save_index"]),
        ("synapse.llm.registries", ["load_custom_providers", "save_custom_providers"]),
        ("synapse.workspace.backup", ["read_backup_settings", "write_backup_settings"]),
        ("synapse.hub.device", ["get_or_create_device_id"]),
        ("synapse.core.identity", ["_load_hashes", "_save_hashes"]),
        ("synapse.orgs.manager", ["load_state", "save_state"]),
    ],
)
...
def test_backup_settings_roundtrip(tmp_path):
    from synapse.workspace.backup import read_backup_settings, write_backup_settings
...
def test_identity_hashes_roundtrip(tmp_path):
    from synapse.core.identity import _load_hashes, _save_hashes
...
def test_identity_hashes_recover_from_corruption(tmp_path):
    from synapse.core.identity import _HASH_FILE, _load_hashes, _save_hashes
