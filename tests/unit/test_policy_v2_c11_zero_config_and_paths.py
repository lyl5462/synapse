...
新用户 ``pip install synapse`` 后无 ``identity/POLICIES.yaml``、无环境
...
Synapse 跑在 Win / macOS / Linux. PathSpec 与 ``_path_under`` 必须保证:
...
from synapse.core.policy_v2.safety_immune_defaults import (
...
class TestR518ZeroConfigFirstInstall:
    ...
    def test_zero_config_global_engine_singleton_loads(self, monkeypatch, tmp_path):
        ...
        try:
            ...
            monkeypatch.setattr(
                "synapse.core.policy_v2.global_engine.Path",
                Path,
            )
