...
from synapse.api.routes import health as health_module
from synapse.api.routes import memory_repair as repair_module
from synapse.storage.degraded import registry as degraded_registry
...
def _reset_state(tmp_path: Path, monkeypatch: pytest.MonkeyPatch):
    ...
    monkeypatch.delenv("SYNAPSE_DESKTOP_SESSION_TOKEN", raising=False)
...
class TestDesktopTokenGate:
    """When ``SYNAPSE_DESKTOP_SESSION_TOKEN`` is set, the quarantine
    ...
    def test_quarantine_rejects_missing_desktop_token(
        ...
        monkeypatch.setenv("SYNAPSE_DESKTOP_SESSION_TOKEN", "desktop-secret")
        token = client.get("/api/memory/repair/degraded").json()["confirmation_token"]
        r = client.post(
            "/api/memory/repair/quarantine",
            json={"subsystem": "feedback", "confirmation_token": token},
    ...
    def test_quarantine_accepts_matching_desktop_token(
        ...
        r = client.post(
            "/api/memory/repair/quarantine",
            json={"subsystem": "feedback", "confirmation_token": token},
            headers={"X-Synapse-Desktop-Token": "desktop-secret"},
        )
        assert r.status_code == 200, r.text
...
class TestTokenStatsRegistersUnderTokenTrackingKey:
    ...
    def test_register_helper_uses_shared_key(self) -> None:
        from synapse.api.routes import token_stats as ts
