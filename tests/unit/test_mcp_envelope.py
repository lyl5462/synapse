...
Mirrors how `web_fetch` emits `[SYNAPSE_SOURCE]`: the chat route can
...
from synapse.tools.handlers.mcp import _build_mcp_envelope
...
def test_envelope_marks_success_with_server_and_tool():
    ...
    assert line.startswith("[SYNAPSE_MCP] ")
    payload = json.loads(line[len("[SYNAPSE_MCP] "):])
    assert payload == {
        "status": "ok",
        "server": "github",
        "tool": "list_repos",
