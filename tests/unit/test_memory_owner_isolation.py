...
from synapse.api.routes.memory import router as memory_router
...
def test_v3_migration_backs_up_and_quarantines_legacy_desktop_memory(tmp_path):
    db_path = tmp_path / "old" / "synapse.db"
    ...
    from synapse.memory.storage import _SCHEMA_VERSION

    assert list(
        db_path.parent.glob(f"synapse.db.bak.v2_to_v{_SCHEMA_VERSION}.*")
    )
