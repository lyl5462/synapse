...
from synapse.scheduler.task import ScheduledTask, TaskStatus, TaskType, TriggerType
from synapse.scheduler.triggers import CronTrigger, IntervalTrigger, OnceTrigger, Trigger
...
class TestSystemTaskTimeouts:
    ...
    async def test_system_task_sets_and_resets_background_token_budget(self, monkeypatch):
        from synapse.core.token_tracking import get_token_budget, record_usage
        ...
        monkeypatch.setattr(
            "synapse.config.settings.scheduler_background_token_budget",
            20,
        )
...
class TestTaskAgentProfiles:
    async def test_executor_creates_selected_agent_profile(self, monkeypatch):
        ...
        monkeypatch.setattr("synapse.agents.factory.AgentFactory.create", fake_create)
    ...
    async def test_chat_created_task_inherits_current_agent_profile(self):
        from synapse.tools.handlers.scheduled import ScheduledHandler
