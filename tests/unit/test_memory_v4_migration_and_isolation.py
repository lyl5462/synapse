...
from synapse.memory.storage import _SCHEMA_VERSION, MemoryStorage
...
def test_workspace_resolver_default_behavior(monkeypatch):
    ...
    from synapse.memory.workspace_resolver import (
        LEGACY_DEFAULT_WORKSPACE_ID,
        resolve_memory_workspace_id,
    )

    monkeypatch.delenv("SYNAPSE_DESKTOP_PROJECT_WORKSPACE", raising=False)
...
def test_workspace_resolver_opt_in_project_mode(monkeypatch, tmp_path):
    ...
    monkeypatch.setenv("SYNAPSE_DESKTOP_PROJECT_WORKSPACE", "1")
...
def test_backfill_sentinel_archives_legacy_json_once(tmp_path: Path):
    ...
    from synapse.memory.storage import _instance_registry
...
def test_isolated_memory_md_seed_phase_2b3():
    ...
    # 不应该硬编码全局 Synapse 身份标记
    assert "Synapse 的全局记忆" not in seed
...
def test_apply_memory_isolation_seeds_md_when_missing(tmp_path: Path, monkeypatch):
    ...
    monkeypatch.setattr(
        "synapse.agents.profile.get_profile_store", lambda: fake_store
    )
    ...
    monkeypatch.setattr(
        "synapse.memory.manager.MemoryManager",
...
def test_iter_owned_session_ids_phase_2b5_audit(tmp_path: Path):
    ...
    storage = MemoryStorage(tmp_path / "synapse.db")
...
async def test_global_store_source_blocks_cross_user():
    from synapse.agents.factory import _GlobalStoreSource
