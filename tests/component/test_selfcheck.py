...
from synapse.evolution.log_analyzer import ErrorPattern, LogEntry
...
class TestSelfCheckResilience:
            ...
            "selfcheck_timeout": ErrorPattern(
                ...
                samples=[
                    LogEntry(
                        timestamp=now,
                        level="ERROR",
                        logger_name="synapse.scheduler.executor",
            ...
            "real_error": ErrorPattern(
                ...
                samples=[
                    LogEntry(
                        timestamp=now,
                        level="ERROR",
                        logger_name="synapse.tools.browser",
    ...
    async def test_run_daily_check_saves_partial_report_on_time_budget(
        ...
        (log_dir / "error.log").write_text(
            "2026-03-31 00:00:00,000 - synapse.tools.browser - ERROR - Browser failed\n",
            encoding="utf-8",
        )

        monkeypatch.setattr("synapse.config.settings.project_root", tmp_path)
        checker = SelfChecker(brain=mock_brain)
        ...
        ticks = iter([0.0, 2.0])
        monkeypatch.setattr(
            "synapse.evolution.self_check.time.monotonic",
