...
# the web_search provider refactor (see src/synapse/tools/tool_hints.py).
#
...
# "✅ Synapse 任务完成 / daily research"，体感极差且查不到源头。
#
...
def _disable_desktop_notifications(monkeypatch):
    ...
    try:
        from synapse.core import desktop_notify as _dn
    except Exception:
        return
...
def _isolate_policy_v2_singletons():
    ...
    """
    try:
        from synapse.core.policy_v2.death_switch import (
            reset_death_switch_tracker,
        )
        from synapse.core.policy_v2.session_allowlist import (
            reset_session_allowlist_manager,
        )
        from synapse.core.policy_v2.skill_allowlist import (
