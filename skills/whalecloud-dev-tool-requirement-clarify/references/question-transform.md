...
```bash
# ❌ 错误：抽象分类问题
py question-transform.py --type=single --title="索引优先级的类型" --context="请选择索引优先级类型" --option1="Hash索引" --option2="B+Tree索引"

# ✅ 正确：示例驱动问题
py question-transform.py --type=single --title="优先级的表示方式" --context="假设索引优先级当前为'中'，运维人员希望调整为更高优先级，优先级的表示方式是？" --option1="固定三级：高/中/低" --option2="数字范围1-100自定义" --option3="固定五级" --option4="自定义标签绑定数值"
```
...
```bash
# 业务视角
py question-transform.py --type=single --title="紧急调优先级的业务效果" --context="[业务视角] 假设电商大促期间，DBA将订单索引优先级从'中'紧急调为'高'，期望达到什么业务效果？" --option1="立刻抢占资源" --option2="渐进式提升" --option3="仅影响排队顺序"

# 开发视角
py question-transform.py --type=single --title="调度器状态切换方式" --context="[开发视角] 假设索引优先级从'中'变更为'高'，现有查询调度器的内部状态如何切换？" --option1="修改内存映射表" --option2="重置调度队列" --option3="需重启调度线程"

# 测试视角
py question-transform.py --type=single --title="长事务中的优先级行为" --context="[测试视角] 假设优先级变更过程中，该索引正被一个长事务使用（已执行30分钟），变更后长事务的行为是？" --option1="继续按旧优先级执行" --option2="立即切换可能超时回滚" --option3="暂停等待变更完成后恢复"
```
...
| 行为可通过源码确认 | `boolean`（判断题） | 在 context 中标注证据来源 |
