...
description: "需求澄清技能 - 分析原始需求，基于产品知识体系事实（代码、文档、需求单上下文、历史相似工单方案），生成需要澄清的问题列表，逐轮向用户确认直到需求清晰。"
label: 需求澄清技能
---
...
| `TMP_DIR` | 否 | 临时文件目录，默认 `./.tmp/`，其下存储 `docs/`（架构文档）及 `.questions.json` |
...
2. **完成阶段**：直接输出 Markdown 格式的需求澄清文档（格式见下方「输出内容格式」章节）
...
Phase 4 — 深度澄清：三视角问题生成 + 行为场景生成（最终交付）
```
...
**执行步骤**：
  ...
  0d. 执行 question-transform.py --reset --output_dir {TMP_DIR} 清理之前的问题列表

  0e. 尝试获取产品架构文档，保存到 {TMP_DIR}/docs/（若 `SYNAPSE_URL` 已提供）：
...
**执行步骤**：
  ...
  2d. **需求动机挖掘**（遵循约束 D1 示例驱动）：
        ...
        **历史工单参考**：若 2c 中提取了历史工单的改造原因，将其作为动机挖掘问题的辅助选项或参考提示，例如在 context 中标注"历史上类似需求的改造原因是...，当前需求是否与之一致？"

  2e. 结合用户圈定的模块范围和入口场景，推导关键搜索线索和关键词（用于后续文档/代码分析）

  2f. 生成动机挖掘问题（遵循约束 D1 示例驱动）：
...
**执行步骤**：
  ...
  4f. 识别可通过代码或文档直接确认的问题，降级为判断题（在 context 中标注证据来源）
...
**执行步骤**：
  ...
  5b. **生成行为场景**：将所有已回答的澄清问题转化为 Gherkin 格式的行为场景
        ...
        - 每个场景标注来源（用户确认 / 代码/文档核验）

  5c. 按照「输出内容格式」章节的结构，直接输出 Markdown 格式的需求澄清文档

  5d. 清理中间产物：删除 {TMP_DIR}/.questions.json 文件

**产出**：直接输出的 Markdown 格式需求澄清文档
