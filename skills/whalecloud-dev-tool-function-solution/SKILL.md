...
1. **输出阶段**：按照下方「输出内容格式」汇总正文，**必须**通过 `whalecloud-dev-tool-doc-generate` 落盘为 `{ARCHIVE_DIR}/函数级方案.md`（见 Step 9 与「文档落地」章节）；本技能不得直接使用 `write_file` 写交付物
...
- **模块组织方式**：以 FUNC_ARCH_DOC 中定义的**核心功能模块**（修改模块+新增模块）为逻辑单元，在 **§1.6 模块改造方案** 一章内按模块分小节输出（如 `#### 1.6.1 {模块名称}`、`#### 1.6.2 {模块名称}`），各模块的函数设计清单、伪代码、模块内部调用关系均归入对应小节
- §1.5 数据设计、§1.7 跨模块交互设计 为**全量**内容，不按模块拆文件，可在各条目下标注所属模块
...
Step 0 — 参数校验与环境准备
  0a. 校验必填参数：WORK_ORDER_DIR, PRODUCT_CODE_ROOT, PRODUCT_DOC_ROOT
      ...
      - 推导输出路径：
        ...
        输出文件固定为：`{ARCHIVE_DIR}/函数级方案.md`（由 Step 9 经 doc-generate 落盘）
...
Step 9 — 文档落地（调用 whalecloud-dev-tool-doc-generate）
  9a. 汇总所有章节内容（区分各方案），按照「输出内容格式」生成完整正文
      - 各模块改造内容在 §1.6（及多方案时的 §2.6）中按模块分小节合并输出
      - 正文从 §0 方案概览与对比 起至 §5 附录止，记为 DOCUMENT_BODY（不含模板页眉中的元数据行）
  9b. 组装 CONTEXT_JSON（UTF-8），建议字段：
      - DOCUMENT_BODY：Step 9a 的完整 Markdown 正文（必填）
      - REQUIREMENT_NAME：来自 CLARIFY_DATA 的需求名称或标题
      - PROD、STATUS：若调用方已传入则填入；未传 STATUS 默认 `draft`
      - SCHEME_COUNT：与参数 SCHEME_COUNT 一致（默认 `2`）
      - 可将 CONTEXT_JSON 写入临时文件 `{ARCHIVE_DIR}/.tmp/function_solution_context.json` 后以路径传入
  9c. **Read** `skills/whalecloud-dev-tool-doc-generate/SKILL.md`，按该技能流程执行文档落地：
      - OUTPUT_DIR: `{ARCHIVE_DIR}`
      - OUTPUT: `函数级方案.md`
      - CONTEXT_JSON: Step 9b 的内联 JSON 或 `.json` 文件路径
      - OUTPUT_MODE: `file`（默认）
      - STRICT: `false`（除非已补齐模板必填标量）
  9d. 确认 `{ARCHIVE_DIR}/函数级方案.md` 已生成且中文可读（依赖 doc-generate 写后自检）；落盘失败则**中止**
  9e. *(DEBUG)* 若 DEBUG=true，将 CONTEXT_JSON 路径、输出路径与文件大小信息写入调试日志
```
...
| OUTPUT | `函数级方案.md` |
| 正文字段 | `CONTEXT_JSON.DOCUMENT_BODY` — 含 §0–§5 全部章节 |
...
函数级方案文档通过 `whalecloud-dev-tool-doc-generate` **统一落盘**为 `{ARCHIVE_DIR}/函数级方案.md`。`DOCUMENT_BODY` 按以下结构组织；涉及的所有模块（FUNC_ARCH_DOC 中的修改模块+新增模块）的改造内容**合并在 §1.6 一章**内，按模块分小节输出。
...
结构与方案1相同，每节编号前缀为 `2.`；§2.6 同样以「所有模块合在一章、按模块分小节」方式输出。
