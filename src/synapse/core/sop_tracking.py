...
INSERT INTO token_usage (
    ...
    context_tokens, iteration, channel, user_id, agent_profile_id, estimated_cost, usage_scene
) VALUES (?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?, ?)
"""
...
def _writer_loop(db_path: str) -> None:
    ...
    try:
        ...
        try:
            conn.execute("ALTER TABLE token_usage ADD COLUMN usage_scene TEXT DEFAULT 'unknown'")
            conn.execute("CREATE INDEX IF NOT EXISTS idx_token_usage_usage_scene ON token_usage(usage_scene)")
            conn.commit()
        except Exception:
            pass  # 列已存在则忽略
