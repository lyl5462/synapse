...
前端 bridge 的具体协议（`bridge:theme-change` / `bridge:locale-change` / `window.Synapse.api(...)` 等）见参考插件 `plugins/tongyi-image/ui/dist/index.html` 与 `plugins/seedance-video/ui/dist/index.html`，以及归档参考副本 `plugins-archive/_shared/web-uikit/`。

For the bridge protocol (`bridge:theme-change`, `bridge:locale-change`,
`window.Synapse.api(...)`, etc.) refer to the two first-class plugins
